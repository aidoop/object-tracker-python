...
class IndyDCPClient:
    ...
    def connect(self):
        ...
        try:
            ...
            if True:
                PrintMsg.printStdErr("Connect: Server IP ({ser_ip})".format(
                    ser_ip=self.server_ip))
                # self.__lock.release()
            return True
    ...
    def set_timeout_sec(self, time_out):
        if time_out < 0:
            PrintMsg.printStdErr(
                "Invalid time out setting: {}<0".format(time_out))
    ...
    def check_header(self, req=HeaderCommand(), res=HeaderCommand(), err_code=ERR_NONE):
        ...
        if req_robot_name != res_robot_name:
            PrintMsg.printStdErr("Header check fail (robotName): Request {_req}, Response {_res}".format(
                _req=req_robot_name, _res=res_robot_name))
        if req.val.stepInfo != res.val.stepInfo:
            PrintMsg.printStdErr("Header check fail (stepInfo): Request {_req}, Response {_res}".format(
                _req=req.val.stepInfo, _res=res.val.stepInfo))
        if req.val.invokeId != res.val.invokeId:
            PrintMsg.printStdErr("Header check fail (invokeId): Request {_req}, Response {_res}".format(
                _req=req.val.invokeId, _res=res.val.invokeId))
        if res.val.sof != self.__sof_server:
            PrintMsg.printStdErr("Header check fail (sof): Request {_req}, Response {_res}".format(
                _req=self.__sof_server, _res=res.val.sof))
        if req.val.cmdId != res.val.cmdId:
            PrintMsg.printStdErr("Header check fail (cmdId): Request {_req}, Response {_res}".format(
    ...
    def _handle_command(self, cmd, req_data=Data(), req_data_size=0):
        ...
        memmove(req_header.val.robotName, c_char_p(
            b_str_robot_name), len(self.robot_name))

        b_str_robot_ver = self.robot_version.encode('ascii')
        memmove(req_header.val.robotVersion, c_char_p(
            b_str_robot_ver), len(self.robot_version))
        ...
        if res_data_size > SIZE_DATA_TCP_MAX or res_data_size < 0:
            PrintMsg.printStdErr("Response data size is invalid {} (max: {}): Disconnected".format(
...
# gwkim
    ...
    def get_last_emergency_info(self):
        ...
        if error_code:
            ...
            memmove(addressof(ret_int_arr), addressof(
                _res_data.byte) + 4, 4 * 3)
            memmove(addressof(ret_double_arr),
                    addressof(_res_data.byte) + 16, 8 * 3)
...
# gwkim
    ...
    def set_endtool_do(self, endtool_type, val):
        ...
        memmove(addressof(data.byte) + 4,
                pointer(c_ubyte(val)), sizeof(c_ubyte))
    ...
    def read_direct_variable(self, dv_type, dv_addr):
        ...
        error_code, _res_data, _res_data_size = self._handle_command(
            CMD_READ_DIRECT_VARIABLE, _req_data, _req_data_size)
    ...
    def read_direct_variables(self, dv_type, dv_addr, dv_len):
        ...
        if dv_len > 20:
            PrintMsg.printStdErr(
                "Length should be less than 20, but {}".format(dv_len))
            return
            ...
            if dv_type == DIRECT_VAR_TYPE_BYTE:  # B
                ...
                if _res_data_size == 2*dv_len:
                    ...
                    for dv_n in range(0, dv_len):
                        ...
                        res.append(int.from_bytes(
                            val, byteorder='little', signed=True))
                    return res
                ...
                if _res_data_size == 2*dv_len:
                    ...
                    for dv_n in range(0, dv_len):
                        ...
                        res.append(int.from_bytes(
                            val, byteorder='little', signed=False))
                    return res
            else:
                PrintMsg.printStdErr("None matched type")
                return False
    ...
    def write_direct_variable(self, dv_type, dv_addr, val):
        ...
        self._handle_command(CMD_WRITE_DIRECT_VARIABLE,
                             _req_data, _req_data_size)
    ...
    def write_direct_variables(self, dv_type, dv_addr, dv_len, val):
        ...
        if dv_type == DIRECT_VAR_TYPE_BYTE:
            for ii in range(0, dv_len):
                memmove(addressof(_req_data.byte) + 12 +
                        1*ii, pointer(c_uint8(val[ii])), 1)
                _req_data_size += 1

        elif dv_type == DIRECT_VAR_TYPE_WORD:
            for ii in range(0, dv_len):
                type_size = 2
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_int16(val[ii])), type_size)
            ...
            for ii in range(0, dv_len):
                type_size = 4
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_int32(val[ii])), type_size)
                _req_data_size += type_size
            ...
            for ii in range(0, dv_len):
                type_size = 8
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_int64(val[ii])), type_size)
                _req_data_size += type_size
            ...
            for ii in range(0, dv_len):
                type_size = 4
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_float(val[ii])), type_size)
                _req_data_size += type_size
            ...
            for ii in range(0, dv_len):
                type_size = 8
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_double(val[ii])), type_size)
                _req_data_size += type_size
            ...
            for ii in range(0, dv_len):
                type_size = 2
                memmove(addressof(_req_data.byte) + 12 + type_size *
                        ii, pointer(c_uint16(val[ii])), type_size)
                _req_data_size += type_size
        else:
            PrintMsg.printStdErr("None matched type")

        self._handle_command(CMD_WRITE_DIRECT_VARIABLES,
                             _req_data, _req_data_size)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        PrintMsg.printStdErr(
            '{0} <Server IP> <Robot Name>'.format(sys.argv[0]))
        sys.exit()
