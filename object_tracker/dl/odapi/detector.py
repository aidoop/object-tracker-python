...
class ObjectDetector:
    ...
    def detect_for_image(self, img):
        ...
        det_boxes = self.extract_bboxes(bboxes, bclasses, bscores, im_width, im_height)

        return det_boxes

    def extract_bboxes(self, bboxes, bclasses, bscores, im_width, im_height):
        bbox = []
    ...
    def get_bboxes_on_image(self, image, boxes_list, det_time=None):
        if not boxes_list:
        ...
        if det_time != None:
            fps = round(1000.0 / det_time, 1)
            fps_txt = str(fps) + " FPS"
            cv2.putText(
                img, fps_txt, (25, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2
            )
