...
class ROIRetangleManager(ROIManager):
    ...
    def isInsideROI(self, newRegion):
        ...
        for idx in range(len(self.RectangleROIList)):
            ...
            if(isInsideEach == True):
                ...
                # if object is in any roi region, 'isInside' should be True as a return value.
                isInside = True
    ...
    def contains(self, newRegion, ROIRegion):
        topA = min(newRegion[0][0], newRegion[1][0],
                   newRegion[2][0], newRegion[3][0])
        bottomA = max(newRegion[0][0], newRegion[1][0],
                      newRegion[2][0], newRegion[3][0])
        leftA = min(newRegion[0][1], newRegion[1][1],
                    newRegion[2][1], newRegion[3][1])
        rightA = max(newRegion[0][1], newRegion[1][1],
                     newRegion[2][1], newRegion[3][1])
