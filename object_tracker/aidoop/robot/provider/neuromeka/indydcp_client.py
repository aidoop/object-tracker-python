...
class IndyDCPClient:
    ...
    def get_last_emergency_info(self):
        ...
        if error_code:
            ...
            return (
                np.array(ret_code).tolist(),
                np.array(ret_int_arr).tolist(),
                np.array(ret_double_arr).tolist(),
            )
    ...
    def read_direct_variables(self, dv_type, dv_addr, dv_len):
        ...
        error_code, _res_data, _res_data_size = self._handle_command(
            CMD_READ_DIRECT_VARIABLES, _req_data, _req_data_size
        )
    ...
    def write_direct_variables(self, dv_type, dv_addr, dv_len, val):
        ...
        if dv_type == DIRECT_VAR_TYPE_BYTE:
            for ii in range(0, dv_len):
                memmove(
                    addressof(_req_data.byte) + 12 + 1 * ii,
                    pointer(c_uint8(val[ii])),
                    1,
                )
                _req_data_size += 1
            ...
            for ii in range(0, dv_len):
                type_size = 2
                memmove(
                    addressof(_req_data.byte) + 12 + type_size * ii,
                    pointer(c_int16(val[ii])),
                    type_size,
                )
    ...
    def move_ext_traj_bin(self, traj_type, traj_freq, dat_size, traj_data, dat_num=3):
        ...
        self._handle_extended_command(
            EXT_CMD_MOVE_TRAJ_BY_DATA, req_ext_data, req_ext_data_size
        )
    ...
    def move_ext_traj_txt(self, traj_type, traj_freq, dat_size, traj_data, dat_num=3):
        ...
        self._handle_extended_command(
            EXT_CMD_MOVE_TRAJ_BY_TXT_DATA, req_ext_data, req_ext_data_size
        )
    ...
    def move_ext_traj_bin_file(self, file_name):
        ...
        self._handle_extended_command(
            EXT_CMD_MOVE_TRAJ_BY_FILE, req_ext_data, req_ext_data_size
        )
    ...
    def move_ext_traj_txt_file(self, file_name):
        ...
        req_ext_data_size = len(file_name)
        self._handle_extended_command(
            EXT_CMD_MOVE_TRAJ_BY_TXT_FILE, req_ext_data, req_ext_data_size
        )
    ...
    def set_and_start_json_program(self, json_string):
        ...
        self._handle_extended_command(
            EXT_CMD_SET_JSON_PROG_START, req_ext_data, req_ext_data_size
        )
