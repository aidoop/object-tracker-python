...
class pyKinectAzure:
    ...
    def bodyTracker_start(self, bodyTrackerModulePath):
        ...
        self.body_tracker = kinectBodyTracker(
            bodyTrackerModulePath, depthSensorCalibration
        )
    ...
    def bodyTracker_get_body_segmentation(self):
        ...
        body_image = self.image_convert_to_numpy(
            self.body_tracker.segmented_body_img
        ).astype(np.uint8)

        # Add color to the segmentation based on the id value of each pixel
        body_image_color = np.dstack(
            [cv2.LUT(body_image, _k4abt.body_colors[:, i]) for i in range(3)]
        )
    ...
    def device_open(self, index=0):
        ...
        """
        _k4a.VERIFY(
            self.k4a.k4a_device_open(index, self.device_handle),
    ...
    def device_get_serialnum(self):
        ...
        result = self.k4a.k4a_device_get_serialnum(
            self.device_handle, None, serial_number_size
        )
        ...
        _k4a.VERIFY(
            self.k4a.k4a_device_get_serialnum(
                self.device_handle, serial_number, serial_number_size
            ),
    ...
    def device_start_cameras(self, device_config=None):
        ...
        if not self.cameras_running:
            _k4a.VERIFY(
                self.k4a.k4a_device_start_cameras(
                    self.device_handle, self.config.current_config
                ),
    ...
    def device_start_imu(self):
        ...
        if self.cameras_running:
            if not self.imu_running:
                _k4a.VERIFY(
                    self.k4a.k4a_device_start_imu(self.device_handle),
    ...
    def device_get_capture(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        if self.cameras_running:
            _k4a.VERIFY(
                self.k4a.k4a_device_get_capture(
                    self.device_handle, self.capture_handle, timeout_in_ms
                ),
                "Get capture failed!",
            )
    ...
    def device_get_imu_sample(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        if self.imu_running:
            _k4a.VERIFY(
                self.k4a.k4a_device_get_imu_sample(
                    self.device_handle, self.imu_sample, timeout_in_ms
                ),
                "Get IMU failed!",
            )
    ...
    def device_get_calibration(self, depth_mode, color_resolution, calibration):
        ...
        """
        _k4a.VERIFY(
            self.k4a.k4a_device_get_calibration(
                self.device_handle, depth_mode, color_resolution, calibration
            ),
    ...
    def image_create(
        self, image_format, width_pixels, height_pixels, stride_bytes, image_handle
    ):
        """Create an image.
        ...
        """
        _k4a.VERIFY(
            self.k4a.k4a_image_create(
                image_format, width_pixels, height_pixels, stride_bytes, image_handle
            ),
            "Create image failed!",
        )
    ...
    def image_convert_to_numpy(self, image_handle):
        ...
        if image_format == _k4a.K4A_IMAGE_FORMAT_COLOR_MJPG:
            ...
            yuv_image = np.frombuffer(buffer_array, dtype=np.uint8).reshape(
            ...
            return np.frombuffer(buffer_array, dtype=np.uint8).reshape(
            ...
            return np.frombuffer(buffer_array, dtype="<u1").reshape(
                image_height, image_width
            )
    ...
    def transform_depth_to_color(self, input_depth_image_handle, color_image_handle):
        ...
        self.device_get_calibration(
            self.config.depth_mode, self.config.color_resolution, calibration
        )
    ...
    def get_imu_sample(self, timeout_in_ms=_k4a.K4A_WAIT_INFINITE):
        ...
        buffer_array = np.array(
            np.ctypeslib.as_array(
                self.imu_sample, shape=(_k4a.IMU_SAMPLE_SIZE,)
            ).tolist()
        )
